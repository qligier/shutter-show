# [ShutterShow](https://react.dev/) &middot; [![workflow status](https://github.com/qligier/shutter-show/actions/workflows/test.yml/badge.svg)](https://github.com/qligier/shutter-show/actions/workflows/test.yml) [![license](https://img.shields.io/badge/license-MIT-blue.svg)](MIT)

ShutterShow is a simple but visually pleasing web application that allows users to display a slideshow of photos and videos.

## Features

- Read photos & videos directly from your local filesystem, no upload required;
- Navigate through medias using the <kbd>left arrow</kbd> and <kbd>right arrow</kbd> keys;
- Display or hide common metadata (such as aperture, shutter speed and ISO) with the <kbd>space</kbd> key;
- Open GoogleMaps at the photo's location with the <kbd>M</kbd> key (if GPS data is available).

## Demo

![Demo of the app usage](https://github.com/qligier/shutter-show/raw/refs/heads/master/.github/demo.avif)